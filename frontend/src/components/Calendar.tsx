...
const SimpleCalendar: React.FC<{ userId: string }> = ({ userId }) => {
  ...
  // Load suggested events from emails
  useEffect(() => {
    axios
      .get(`${BASE_URL}/api/emails?user_id=${userId}&limit=100`)
      .then((res) => {
        ...
        emails.forEach((email: any) => {
          ...
          if (
            ...
            !events.some(
              (evt) =>
                evt.title === email.event_details.title &&
                evt.datetime === email.event_details.datetime
            )
          ) {
            suggestions.push({
              type: "event",
              title: email.event_details.title || "Event",
              datetime: email.event_details.datetime,
              end_datetime: email.event_details.end_datetime,
          ...
          if (
            email.task_details &&
            email.task_details.tasks &&
            email.task_details.tasks.length > 0
          ) {
            email.task_details.tasks.forEach((task: any) => {
              if (task.due_date) {
                suggestions.push({
                  type: "task",
  ...
  // Approve suggested event/task
  const handleApprove = async (suggestion: any) => {
    try {
      await axios.post(`${BASE_URL}/api/calendar/events`, {
        user_id: userId,
      ...
      // Remove from suggestions and refresh calendar events
      setSuggestedEvents((prev) => prev.filter((s) => s !== suggestion));
      // Optionally, reload calendar events here
    } catch {
      alert("Failed to approve event.");
    }
  };

  // Deny just removes from suggestions
  const handleDeny = (suggestion: any) => {
    setSuggestedEvents((prev) => prev.filter((s) => s !== suggestion));
  };
    ...
    <div className="calendar-container">
      ...
      <div className="events-section">
        ...
        <div className="add-event-section">
          ...
          {/* Suggested Events Subsection */}
          <div className="suggested-events-section" style={{ marginTop: 24 }}>
            <h3>Suggested Events</h3>
            {suggestedEvents.length === 0 ? (
              <div>No suggested events found from your emails.</div>
            ) : (
              <ul>
                {suggestedEvents.map((s, idx) => (
                  <li key={idx} style={{ marginBottom: 12 }}>
                    ...
                    <span>
                      {s.datetime && (
                        <>
                          <strong>When:</strong> {new Date(s.datetime).toLocaleString()}
                          <br />
                        </>
                      )}
                      {s.location && (
                        <>
                          <strong>Where:</strong> {s.location}
                          <br />
                        </>
                      )}
                      {s.description && (
                        <>
                          <strong>Summary:</strong> {s.description}
                          <br />
                        </>
                      )}
                    </span>
                    ...
                    <button onClick={() => handleDeny(s)}>Deny</button>
                  </li>
                ))}
              </ul>
            )}
          ...
          {dayViewDate && (
            <div style={{ maxHeight: 400, overflowY: "auto", border: "1px solid #404040", marginTop: 16, padding: 8 }}>
              <h3>
                ...
                <button style={{ marginLeft: 8 }} onClick={() => setDayViewDate(null)}>
                  Close
                </button>
              </h3>
